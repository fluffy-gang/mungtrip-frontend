# 멍멍로드 Play 심사 최소 버전 Architecture

## 현재 구조

앱은 Expo SDK 57, React Native 0.86, Expo Router와 네이버 지도 네이티브 SDK를 사용한다. `src/app`의 얇은 route가 `src/features/home`과 `src/features/places` 화면을 조합하고, `src/shared/api`의 Axios 클라이언트가 멍멍로드 API를 호출한다.
...
- `src/features/home`: 제주 기본 영역 지도, 카테고리, 익명 검색과 장소 목록 상태를 관리한다.
...
1. 앱은 고정된 제주 bounds로 카테고리와 장소 목록을 익명 요청한다.
2. API 서버는 한국관광공사 기반 장소 데이터를 앱 표시 타입으로 반환한다.
3. 앱은 좌표가 있는 장소만 지도 마커로 표시하고 전체 유효 응답을 목록에 표시한다.
4. 검색어는 요청 시에만 서버로 전송하고 앱과 서버에 영구 저장하지 않는다.
5. 장소 선택 시 ID로 상세를 요청하고 존재하는 필드만 렌더링한다.
6. 네이버 지도 SDK는 지도 렌더링에만 사용하며 사용자 위치를 전달하지 않는다.

## 시스템 경계

- 멍멍로드 API: 장소 데이터와 검색 결과를 제공하는 외부 운영 경계다.
